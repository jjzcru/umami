...
export function getDateQuery(field, unit, timezone, type = '') {
  const db = getDatabase();

  if (db === POSTGRESQL) {
    // TODO If you are using postgres and not cockroach db replace to_char_with_style with to_char
    if (timezone) {
      if (type === 'stats') {
        return `to_char(date_trunc('${unit}', ${field} at time zone '${timezone}')::TIMESTAMP)`;
      }
      return `to_char_with_style(date_trunc('${unit}', ${field} at time zone '${timezone}')::TIMESTAMP, '${POSTGRESQL_DATE_FORMATS[unit]}')`;
    }
    if (type === 'stats') {
      return `to_char(date_trunc('${unit}', ${field})::TIMESTAMP)`;
    }
    return `to_char_with_style(date_trunc('${unit}', ${field})::TIMESTAMP, '${POSTGRESQL_DATE_FORMATS[unit]}')`;
  }
    ...
    `
      ...
      from (
         select pageview.session_id,
           ${getDateQuery('pageview.created_at', 'hour', null, 'stats')},
           count(*) c,
...
export function getPageviewStats(
  ...
  return rawQuery(
    `
    select ${getDateQuery('pageview.created_at', unit, timezone, 'stats')} t,
      count(${count}) y
    from pageview
      ${joinSession}
    where pageview.website_id=$1
